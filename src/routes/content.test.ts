...
describe('content routes', () => {
  describe('shareContentSchema', () => {
    ...
    it('should accept a valid payload without path', () => {
      const result = shareContentSchema.safeParse(validPayload);
      expect(result.success).toBe(true);
    });

    it('should accept a valid payload with path', () => {
      const result = shareContentSchema.safeParse({ ...validPayload, path: '/blog/my-post' });
      expect(result.success).toBe(true);
    });

    it('should reject payload with empty type', () => {
      const result = shareContentSchema.safeParse({ ...validPayload, type: '' });
    ...
    it('should reject payload with empty documentCid', () => {
      const result = shareContentSchema.safeParse({ ...validPayload, documentCid: '' });
      expect(result.success).toBe(false);
    });

    it('should reject payload with empty title', () => {
      const result = shareContentSchema.safeParse({ ...validPayload, title: '' });
      expect(result.success).toBe(false);
    });
