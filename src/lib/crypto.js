 ...
 */
function getKey() {
  const hex = config_1.config.encryptionKey;
...
function hashApiKey(apiKey) {
  const salt = crypto_1.default.randomBytes(API_KEY_SALT_LENGTH);
  const derivedKey = crypto_1.default.scryptSync(
    apiKey,
    salt,
    API_KEY_KEY_LENGTH,
    {
...
function verifyApiKey(rawKey, storedHash) {
  try {
    ...
    return crypto_1.default.timingSafeEqual(derivedKey, storedKey);
  } catch {
